...
export function MunicipioGruposPage() {
    ...
    useEffect(() => {
        Promise.all([fetchMunicipios(), refetchGrupos()]).finally(() => setInitialLoading(false))
    }, [])
    ...
    const handleAddSubmit = async (e: React.FormEvent) => {
        ...
        setIsLoading(true)
        try {
            await addGrupo({ nome: formData.nome, municipioId: formData.municipioId ? Number(formData.municipioId) : munId })
            await refetchGrupos()
                                        ...
                                        <tr key={grupo.id}>
                                            ...
                                            <td className="align-middle text-end">
                                                <div className="btn-group btn-group-sm">
                                                    <button className="btn btn-outline-primary" onClick={() => { setSelectedGrupo(grupo); setFormData({ nome: grupo.nome, municipioId: grupo.municipioId ? String(grupo.municipioId) : munId ? String(munId) : '' }); setFormErrors({}); setShowEditModal(true) }}><i className="bi bi-pencil"></i></button>
