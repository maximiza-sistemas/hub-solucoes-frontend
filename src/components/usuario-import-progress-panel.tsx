...
function exportarFalhas(progress: ImportJobProgress) {
    const rows: string[][] = [['linha', 'usuario', 'motivo']]
...
export function UsuarioImportProgressPanel() {
    const { progress, fileName, cancelling, cancel, dismiss } = useUsuarioImportJobStore()
                ...
                <div className="p-3" style={bodyStyle}>
                    ...
                    {status === 'CANCELLED' && (
                        <div className="alert alert-warning py-2 mb-3 small">
                            ...
                            Cancelada após {progress.sucesso} de {progress.totalLinhas} usuários.
                        </div>
                    )}
                        ...
                        <div className="mt-3">
                            {!confirmCancel ? (
                                ...
                                <div className="border border-warning rounded p-2 small">
                                    <div className="mb-2">
                                        Tem certeza? Os usuários já cadastrados serão mantidos.
                                    </div>
        ...
        <>
            <ul className="nav nav-tabs nav-fill mb-2 small">
                <li className="nav-item">
                    <button className={`nav-link ${tab === 'cadastrados' ? 'active' : ''}`} onClick={() => setTab('cadastrados')}>
                        Cadastrados ({progress.sucesso})
                    </button>
                </li>
                <li className="nav-item">
                    <button className={`nav-link ${tab === 'existentes' ? 'active' : ''}`} onClick={() => setTab('existentes')}>
                        Já cadastrados ({progress.existentes})
                    </button>
                </li>
                <li className="nav-item">
                    <button className={`nav-link ${tab === 'falhas' ? 'active' : ''}`} onClick={() => setTab('falhas')}>
